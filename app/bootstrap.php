...
/**
 * 采集器统一引导文件。
 * cj_html 下的 Web 入口与 app/bin 下的 CLI 脚本都从这里启动。
...
// PSR-4 风格自动加载：Cj\Foo\Bar → app/src/Foo/Bar.php
...
function cj_config(?string $key = null)
{
    ...
    if ($config === null) {
        ...
        if (!is_file($file)) {
            fwrite(STDERR, "缺少配置文件 app/config/config.php（可从 config.example.php 复制）\n");
            exit(1);
        }
        $config = require $file;
    }
